...
The project uses the _monorepo_ approach, so the ASTx library is stored at
**packages/astx**.

In order to be able to contribute, it is important that you understand the
project layout. This project uses the _src layout_, which means that the ASTx
package code is mainly located at `./packages/astx/src/astx`.
